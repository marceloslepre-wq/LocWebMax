onRecordAfterUpdateSuccess((e) => {
  try {
    var oldContract = e.record.original().getString('contract_number')
    var newContract = e.record.getString('contract_number')
    if (oldContract !== '' || newContract === '') return e.next()
    ...
    var tplCfg = templates['novo_contrato']
    ...
    var contractNumber = newContract
    ...
    var totalVal = String(rental.get('total') || 0)
    ...
    try {
      ...
      $app
        .logger()
        .error(
          'WhatsApp notification (novo_contrato) failed',
          'err',
          sendErr.message || String(sendErr),
        )
    }
  } catch (err) {
    $app.logger().error('notification_rental_create error', 'err', err.message || String(err))
  }
  return e.next()
}, 'rentals')
