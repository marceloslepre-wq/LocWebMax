onRecordAfterUpdateSuccess((e) => {
  try {
    ...
    if (oldStatus === 'Devolvido' || newStatus !== 'Devolvido') return e.next()
    ...
    var tplCfg = templates['confirmacao_devolucao']
    ...
    try {
      ...
      $app
        .logger()
        .error(
          'WhatsApp notification (confirmacao_devolucao) failed',
          'err',
          sendErr.message || String(sendErr),
        )
    }
  } catch (err) {
    $app.logger().error('notification_rental_return error', 'err', err.message || String(err))
  }
  return e.next()
}, 'rentals')
