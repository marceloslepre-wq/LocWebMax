...
migrate(
  (app) => {
    // 1. Ensure helena cron job is paused in _cron_settings if table exists
    try {
      if (app.hasTable('_cron_settings')) {
        const cronCols = app.findCollectionByNameOrId('_cron_settings')
        let settingRec = null
        try {
        ...
        if (settingRec) {
          ...
          settingRec.set(
            'note',
            'Pausado aguardando validação das novas mensagens e fluxo PIX pelo Marcelo',
          )
          app.save(settingRec)
        } else {
          const newSetting = new Record(cronCols)
          newSetting.set('job', 'helena_daily_cobranca')
          newSetting.set('disabled', true)
          newSetting.set(
            'note',
            'Pausado aguardando validação das novas mensagens e fluxo PIX pelo Marcelo',
          )
      ...
      console.log('0056: Note on pausing helena_daily_cobranca:', cronErr)
    }

    // 2. Redefine agent Helena with strict instructions:
    // - NEVER invent payment data, static PIX keys, CNPJs, or bank accounts
    // - Use exact values provided in the prompt/context
    // - Special products rule & scope rule
...
REGRA 2 - VALOR CORRETO DA RENOVAÇÃO:
- NUNCA use o valor acumulado histórico da locação para orçar renovações.
- O valor da renovação é sempre baseado no "Valor Mensal" do produto no Estoque, que vem injetado e especificado no bilhete/instrução do sistema.
- 30 dias = Valor Mensal cheio do produto.
- 15 dias = Metade do Valor Mensal do produto (quando o produto permitir 15 dias).
- Use SOMENTE os valores exatos de 15 dias e 30 dias especificados no contexto da mensagem. Nunca adivinhe ou altere valores.
