migrate(
  (app) => {
    // Migration 0070: Resolução Final e Definitiva de Itens de Locação
    //
    // Contexto: As migrations 0067, 0068 e 0069 falharam em atualizar de fato os contratos
    // porque o match de valor era rígido (exigia igualdade exata sem tolerância ao frete embutido)
    // ou usava app.db().newQuery().bind(...) que não persistia no PocketBase orm / SQLite do container.
    //
    // Regras Desta Migration:
    // 1. Resolução em cascata por ordem de confiança:
    //    a) itemId / item_id existente e válido no inventário;
    //    b) code / SKU exato no texto ou item;
    //    c) referência numérica no texto (ex: "(840)", "ref 840", "cód 344");
    //    d) nome exato e palavras-chave significativas;
    //    e) histórico em rental_snapshots e auditoria_contratos;
    //    f) DEDUÇÃO POR VALOR COM TOLERÂNCIA A FRETE:
    //       Seja P = total do contrato / número de itens não-frete, ou monthlyPrice do item.
    //       Para contratos > 30 dias (ex: 90 dias), base = P / (dias / 30).
    //       Case com produto no inventário onde:
    //         product.monthlyPrice <= base && (base - product.monthlyPrice) <= 100
    //         OU Math.abs(base - product.monthlyPrice) <= 0.01
    //       Escolha o produto de MAIOR monthlyPrice que satisfaça (preferir o mais caro plausível).
    // 2. Regras explícitas garantidas para casos reais citados:
    //    - LOC-00537 (total 550, 30d): Cama 03 Movimentos Motorizada (cód 840, R$ 500/mês + R$ 50 frete)
    //    - LOC-00534 (total 240, 30d): Cadeira de Rodas 120kg Desmontável Tam 44 (cód 344, R$ 190/mês + R$ 50 frete)
    //    - LOC-00119 e LOC-00114 (total 950, 90d): Cama 03 Movimentos Manual (cód 830, R$ 300/mês x 3 + R$ 50 frete)
    // 3. Ao resolver, preencher: itemId, item_id, code, name (nome real do produto),
    //    dailyPrice, daily_price (do cadastro), monthlyPrice, monthly_price (do cadastro), totalPrice.
    //    NUNCA alterar datas, quantidades ou remover itens. Contagem final >= inicial.
    // 4. Se um item NÃO puder ser resolvido com confiança:
    //    Preservar o item como está e registrar em pendências.
    //    Gravar na coleção "logs_suporte_master" (ou logs_suporte se existir) um registro
    //    indicando a pendência com número do contrato, id, item problemático e motivo.
    // 5. Limpar custom_contract_html e custom_contract_text dos contratos corrigidos.
    // 6. Persistência garantida: usar app.save(rental) para que o PocketBase valide e grave
    //    diretamente a collection com todos os hooks e triggers.

    console.log('Iniciando Migration 0070: Resolução Final de Itens de Locação...')
    ...
    try {
      ...
      console.log('Erro ao carregar inventário na migration 0070: ' + e.message)
      return
    }
    ...
    // Ordenar invList por monthlyPrice decrescente para favorecer o mais caro plausível
    invList.sort(function (a, b) {
      return b.monthlyPrice - a.monthlyPrice
    })

    // Referências rápidas para os produtos citados
    ...
    // Função de resolução em cascata por ordem de confiança
    var resolveProduct = function (
      ...
      // Seja P = itemPrice (ou contractTotal). Se período > 30 dias (ex: 90 dias), base = P / (dias / 30)
      ...
      if (targetMonthly > 0) {
        ...
        // Como invList já está ordenado por monthlyPrice decrescente, o primeiro que satisfizer
        // é o de maior monthlyPrice plausível (preferindo o mais caro plausível).
        // Restrição: só deduzir se o produto tiver valor relevante (>= 50) para evitar casar frete de 100 com item de 10
        for (var ft = 0; ft < invList.length; ft++) {
          var cand = invList[ft]
          if (cand.monthlyPrice <= 0) continue
          var diff = baseMonthly - cand.monthlyPrice
          if (diff >= 0 && diff <= 100) {
            // Se a diferença for > 0, certificar que cand.monthlyPrice é expressivo em relação a diff
    ...
    // 4. Carregar todos os rentals
    var allRentals = []
    try {
      allRentals = app.findRecordsByFilter('rentals', "id != ''", 'created', 0, 0)
    } catch (e) {
      console.log('Erro ao buscar rentals na migration 0070: ' + e.message)
      return
    }

    console.log('Total de rentals a processar na migration 0070: ' + allRentals.length)

    var countCorrected = 0
    var countAlreadyOk = 0
    var pendingList = []

    // Obter um usuário master padrão para preencher logs_suporte_master se necessário
    var masterUser = null
    try {
    ...
    for (var r = 0; r < allRentals.length; r++) {
      ...
      if (hasUnresolvedItem && masterUser) {
        try {
          ...
          logRec.set('master_email', masterUser.getString('email') || 'master@locwebpro.com')
          logRec.set('master_name', masterUser.getString('name') || 'Master')
          var tId = rental.getString('tenant_id')
          if (!tId) {
            // Obter qualquer tenant existente para preencher campo obrigatório
            try {
          ...
          if (tId) {
            ...
            logRec.set(
              'user_agent',
              'Migration 0070 Pendência Contrato ' + cNumber + ': item não resolvido com confiança',
            )
            app.save(logRec)
          }
        } catch (logErr) {
          console.log(
            'Aviso ao registrar pendência em logs_suporte_master para ' +
              cNumber +
              ': ' +
              logErr.message,
          )
      ...
      if (newItems.length >= rawItems.length && newItems.length > 0 && itemsModified) {
        ...
        try {
          ...
          console.log(
            'Contrato ' +
              cNumber +
              ' corrigido com sucesso na 0070. Itens: ' +
              newItems
                .map(function (x) {
          ...
          console.log('Erro ao executar app.save no rental ' + cNumber + ': ' + saveErr.message)
        }
      } else {
        countAlreadyOk++
      }
    ...
    console.log('=== RESUMO DA MIGRATION 0070 ===')
    console.log('Total de contratos corrigidos e salvos no banco: ' + countCorrected)
    console.log('Total de contratos mantidos completos/já resolvidos: ' + countAlreadyOk)
    console.log('Total de pendências registradas para revisão manual: ' + pendingList.length)
    if (pendingList.length > 0) {
      console.log('Pendências: ' + JSON.stringify(pendingList))
    }
  },
  (app) => {
