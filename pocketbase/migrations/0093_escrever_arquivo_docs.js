migrate(
  (app) => {
    // Migration 0093: Gravar o arquivo docs/contratos-ativos-sem-fonte.txt diretamente via filesystem Go
      ...
      if (!sourceItems || sourceItems.length === 0) {
        ...
        for (var it = 0; it < rawItems.length; it++) {
          ...
          parsedItems.push({
            ...
            code: itemObj.code || '',
            name: itemObj.name || itemObj.productName || itemObj.description || '',
    ...
    lines.push('Critério: Sem snapshot em rental_snapshots e sem itens em auditoria_contratos')
    ...
    for (var i = 0; i < noSourceActiveList.length; i++) {
      ...
      if (!item.its || item.its.length === 0) {
        lines.push('   Itens: [Nenhum item]')
      } else {
        ...
        for (var j = 0; j < item.its.length; j++) {
          ...
          lines.push(
            '     - Código: ' +
              ...
              it.qty +
              ' | Valor Mensal: R$ ' +
              it.monthlyPrice.toFixed(2) +
              ' | Diária: R$ ' +
              it.dailyPrice.toFixed(2) +
              ' | Total Item: R$ ' +
    ...
    // Tentar escrever no arquivo docs/contratos-ativos-sem-fonte.txt
    try {
      $os.writeFile('docs/contratos-ativos-sem-fonte.txt', fullTxt, 438) // 0666 octal = 438 dec
      console.log(
        '[Migration 0093] Arquivo docs/contratos-ativos-sem-fonte.txt escrito com sucesso!',
      )
    } catch (wErr) {
      console.log('[Migration 0093] Erro ao escrever arquivo: ' + wErr.message)
    }
  },
  (app) => {},
)
