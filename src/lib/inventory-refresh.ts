...
export function mapInventoryRecord(item: any) {
  return {
    id: item.id,
    name: item.name,
    code: item.code,
...
export async function refreshStoreInventory() {
  try {
    const data = await pb.collection('inventory').getFullList({ sort: '-created' })
    const mapped = data.map(mapInventoryRecord)
    ...
    // silent fail — realtime will handle it
  }
}
